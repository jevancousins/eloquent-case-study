...
const variants = {
  ...
  ghost: 'bg-transparent text-gray-600 hover:bg-gray-100 hover:text-gray-900 focus:ring-blue-200',
};
...
export default function Button({ children, variant = 'primary', loading = false, disabled = false, onClick, className = '' }) {
  return (
    <button
      ...
      className={`inline-flex items-center justify-center rounded-lg font-medium transition-all focus:outline-none focus:ring-2 focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer px-4 py-2 text-sm ${variants[variant]} ${className}`}
    >
